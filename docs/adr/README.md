...
| [0013](0013-application-owned-persistence-repository-boundary.md) | **Proposed.** Domain-facing, application-owned repository boundaries (one per persisted domain) wrap the existing `localStorage` implementation unchanged, in preparation for a later IndexedDB adapter and a future sync layer above local persistence — see `docs/PERSISTENCE_BOUNDARY_DESIGN.md` |
