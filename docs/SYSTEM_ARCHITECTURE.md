...
`TrainingPlanStepEditor.tsx` and `TrainingPlanExercisePicker.tsx` (one searchable,
catalog-driven Technique / Shotmaking / Measured chooser with descriptive cards and an
inline setup/diagram preview; Release Time then reuses `TrainingSetup.tsx` unmodified for the timing block-
...
ADR-0012 Decision 5). An incomplete Release Time step instead shows its remaining
configured stones at the bottom of the active runner. `Start New Session` and `New
Training Block` are hidden while a plan owns progression, so a middle timing step cannot
strand or silently abandon the following Exercise.
...
## Exercise Library and execution (Stages A-E implemented for the closed beta)
...
- **Implemented:** a versioned curated content package, its validation boundary, the two
  ...
  Release Time Library linkage. ADR-0031 adds Stage C1's standalone Team domain:
  confirmed Profile participants, multiple athlete results, one active recorder, five
  rotation configurations, actual role segments and Team attempt attribution. ADR-0032
  adds executed Postgres schema/RLS/RPCs for recording permission, the shared immutable
  Session envelope, athlete-owned bundles, partial rejection, concrete approval and
  athlete-only private notes. ADR-0033 adds the strict Team serializer, provider-neutral
  and Supabase RPC service, plus schema-v2 Team entries in ADR-0027's existing
  Profile-scoped durable outbox with ordered upload and partial-sync receipts. ADR-0034
  advances that record to schema 3 with a strict active-roster/permission observation
  cache and adds the athlete's own permission control to Team settings. ADR-0035 advances
  ...
  Analyze detail/raw export and own private-note save/clear. ADR-0038 adds durable,
  append-only active-attempt corrections and recorded-by-mistake annulment. ADR-0039
  adds executed Postgres/RLS/RPC authority for athlete-only post-completion correction,
  terminal whole-result void and idempotent metadata notifications. C4b extends the
  provider-neutral service, validates complete owner-only revision chains and advances
  the existing Profile cache to schema 6 with original/current result and void state.
...
- C1 Team aggregates are not accepted by that Solo Session boundary or cloud serializer.
  C2b deliberately reuses the existing Profile-scoped outbox key with separate Team
  envelope/bundle entries; it does not create a Team Session key or widen the Solo
  serializer. C2a deliberately adds a separate Team
  cloud authority instead of widening the recorder-owned `training_session` record.
  Private Athlete Notes are forbidden in the shared recorder aggregate and stored only
  through the athlete-owned SQL boundary. Team Release Time cannot create a parallel
  Measured execution.
...
- `measurementProtocols.ts` — reusable, versioned Measurement Protocols, not fields
  ...
  Hog-Hog exists. C3b adds manual Rotation Count in rotations with no release-time mode
  and a strict 0.5 value increment. `allowedSources` is exactly `["manual"]` and
  `target` is validated as `null`: no curated protocol prescribes a target or tolerance,
  and none implies hardware capture that does not exist.
- `executionTypes.ts` / `execution.ts` — ADR-0028's Solo aggregate and transitions plus
  the shared contracts Stage C1 extends. A Solo execution deep-snapshots its curated
  Version and enabled Measurement Protocols, uses one role segment and one
  athlete-owned result, and never overloads the release-timing `Shot` type. Absence of
  `teamContext` remains the backwards-compatible Solo shape; Team-only recorder/role
  fields cannot appear on it.
- `teamExecution.ts` — Stage C1 creation, Team Shotmaking attempt capture, manual and
  planned role changes, rotation recommendations, active attempt correction/annulment,
  completion/abandonment and rink-order derivation. Planned rotation and attempt-level
  role corrections never overwrite actual role-segment history.
- `executionValidation.ts` — one fail-closed persisted boundary for both shapes. Team
  validation checks roster/result cardinality, role membership, recorder attribution,
  chronological segments, automatic-rotation claims, required deviations, globally
  unique ids, active correction chains and per-athlete attempt sequence together. It
  rejects private Athlete Notes in the shared aggregate and rejects a parallel Team
  Measured execution.
...
### Team cloud authority, upload, offline state, rink UI and owned restore (`supabase/`, `src/lib/cloudSporting/`, Stages C2a-C4c)

ADR-0032 adds three forward-only migrations, deliberately separate from the
Profile-owned `sporting_records` table:

- `20260828120000_team_exercise_cloud_schema.sql` — historical recording permissions,
  immutable Team Session envelopes, relational participant/execution manifests,
  athlete-owned result bundles/result references, concrete Session approvals and
  separate private Athlete Notes;
- `20260828120100_team_exercise_cloud_rls.sql` — SELECT-only authenticated ACL plus
  ownership/eligibility RLS, no browser write policy and no anonymous table access; and
- `20260828120200_team_exercise_cloud_functions.sql` — athlete-owned permission and note
  mutations, server-derived-recorder envelope upload, independent per-athlete bundle
  upload, exact retry/conflict outcomes and concrete-Session approval.

ADR-0039 adds three forward-only C4a migrations:

- `20260828120300_team_exercise_result_revisions_schema.sql` — append-only athlete-owned
  revisions plus a stable source-event key and result-change kind on the existing
  notification inbox;
- `20260828120400_team_exercise_result_revisions_rls.sql` — owner-only revision SELECT
  and no browser table mutation; and
- `20260828120500_team_exercise_result_revisions_functions.sql` — server-derived actor,
  stable-id/base-revision conflict control, terminal voiding and atomic recipient-filtered
  notification/audit emission.

The shared coordination payload and athlete payloads are lossless TEXT with server-owned
SHA-256 digests; UUID-only authority manifests are relational. One athlete-specific
block returns a named bundle outcome without rolling back accepted bundles. Participation
...
- `teamExerciseRecords.ts` validates one completed C1 aggregate and splits recorder-free,
  non-private coordination from one private-note-free result payload per athlete. Cloud
  payload schema 2 keeps active performance corrections out of coordination and filters
  them into only the before/after athlete bundles; schema 1 remains readable;
...
- `syncStateRepository.ts` schema 6 keeps legacy personal `entries`, C2b `teamEntries`
  ...
  strictly validated `teamExerciseResults`. C4b retains immutable original plus current
  result, contiguous post-completion history and terminal void state; schema 5 migrates
  without inventing a revision, schemas 1-3 migrate without a draft and schemas 1-4
  migrate with an empty result cache; and
- `SportingCloudSyncManager` durably writes the full package before upload, sends the
  ...
  pending/fully-synced/partially-blocked/issue truth.

Production composition injects the Team service and exposes upload, eligibility and
active-draft methods bound to the mounted Profile. `TeamsScreen` refreshes the bounded
cache from the active workspace and Team-member RLS permission read, and lets the
authenticated athlete grant or revoke only their own prospective permission. The draft
write rolls back in-memory state on storage failure; exact completion atomically replaces
the draft with the immutable Session/bundle package before upload. ADR-0036's Team setup
uses only cached Teams containing the recorder, derives the recorder from the signed-in
...
and saves/clears the own private note online only after RPC acknowledgement. C3d adds a
durable correction editor for any active stone, audited recorded-by-mistake annulment,
and affected-athlete-only correction history in Analyze. C4a supplies the server
authority for post-completion revisions, terminal whole-result void and participant
notification emission. C4b adds named mutation RPC mappings and fail-closed revision
reads into the existing offline cache. C4c exposes only refreshed owner results for
...
- `diagrams.ts` — independently authored structured diagrams for historical Eight Guards
  Version 2 and Release Gates, plus one generic builder for attributed Swiss Curling
  source diagrams. Optional normalized text overlays cover embedded source-language
  labels with English without branching on an Exercise or asset id.
...
- `validation.ts` — treats the package as untrusted even though it is authored in code,
  ...
  unit, mode or source; or pairs Release Time/Rotation Count with the wrong unit or mode;
  unsupported diagram kinds, element kinds and coordinate systems;
  malformed or out-of-range normalised coordinates; a Diagram missing its English caption
  or accessible summary; and an incomplete source-image distribution. Public diagrams
  must explicitly permit public delivery; restricted diagrams must explicitly forbid it.
...
- `exerciseAssets.ts` / `restrictedAssetCatalog.ts` — the current public diagram
  allowlist, versioned public paths and cache-first browser resolver. The resolver reads
  one `StorageAdapter` key per immutable asset id before fetching, accepts only a
...
- `src/app/api/exercises/restricted-diagrams/[assetId]/route.ts` — retained authenticated private
...
- `authorizedFetch.ts` / `teamServiceFactory.ts` — the retained restricted browser resolver shares the one
  ...
  supplies a data URL only after an authorised response; any refusal or transport error
  becomes the normal unavailable state.
...
- `ExerciseDetail.tsx` — **one generic renderer**, following the specification's fixed
  ...
  a hero (title, classification badges, Exercise version); "Goal" (goal,
  ...
  reference goal); and one grouped container holding Variations and Compatible Measurements
  as progressive-disclosure rows. One compact source footer appears at the very bottom. A Measured Exercise's
...
- **The immutable Exercise Version is shown for every Exercise** ("Exercise version N"
  in the hero). The compact source footer is visually subordinate and does not duplicate
  full provenance metadata already retained in the content model.
...
| `ExerciseDetail.tsx` | The one generic Exercise detail renderer, in the specification's fixed information order across consolidated surfaces; shows the immutable Exercise version, focus-semantic start action and one compact source footer at the bottom; branches only on declared domain semantics, never on an Exercise id or title |
...
| `ExerciseRestrictedSourceImage.tsx` | Resolves an attributed source image asynchronously through the injected public/restricted resolver, suppresses stale prior resolutions, renders generic normalized English text overlays where declared, and otherwise shows the fail-closed unavailable state without exposing the opaque reference (ADR-0023/0044) |
...
| `TrainingPlanExercisePicker.tsx` / `TrainingPlanStepEditor.tsx` | Searchable Technique/Shotmaking/Measured catalogue with descriptive cards and inline setup/diagram preview, shared by create/edit; snapshots the exact selected Exercise Version, while only Release Time wraps `TrainingSetup.tsx` and adds Number of Stones plus Handle Strategy |
...
| `TrainingPlanStepTransition.tsx` | "Continue to next step" mid-plan, or a distinct "Plan complete" + Finish Training on the final step — never both at once; Release Time displays remaining stones until this transition becomes reachable |
...
| Module | Responsibility |
|---|---|
| `types.ts` | `Exercise` (stable identity) vs. `ExerciseVersion` (immutable content), the independent classification dimensions, participation/sweeping/equipment/instruction/variation shapes, the `ExerciseGuidance` and `ExerciseDiagram` unions, public/restricted asset distributions, `MeasurementProtocol`, and `ExerciseCatalogPackage` |
...
| `executionValidation.ts` | Fail-closed untrusted aggregate validation for backwards-compatible Solo and standalone Team shapes; Team recorder/roster/role/rotation/result/privacy/correction-chain invariants are checked together |
| `executionResult.ts` | Pure per-athlete Shotmaking and Measurement summaries; no stored derived result |
| `sessionIntegration.ts` | Strict Stage B embedding in the Profile-owned Session and explicit rejection of C1 Team aggregates until their coordination/athlete-bundle persistence boundary exists |
